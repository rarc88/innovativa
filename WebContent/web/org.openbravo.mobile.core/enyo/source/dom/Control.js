...
enyo.kind({
	...
	*/
	...
	generateChildHtml: function() {
		var results = '';
		for (var i=0, c; (c=this.children[i]); i++) {
			if (OB.resizing && (c.attributes && c.attributes.class && _.isString(c.attributes.class) && c.attributes.class.startsWith('enyo-popup onyx-popup') && !(c.attributes.class.startsWith('enyo-popup onyx-popup onyx-menu')))) {
				results += '';
			} else {
				h = c.generateHtml();
				results += h;
	...
	teardownRender: function() {
		if (OB.resizing && (this.attributes && this.attributes.class && _.isString(this.attributes.class) && this.attributes.class.startsWith('enyo-popup onyx-popup') && !(this.attributes.class.startsWith('enyo-popup onyx-popup onyx-menu')))) {
			return;
		}
		if (this.generated) {
	...
	statics: {
		...
		*/
		...
		registerDomEvents: function(inId, inControl) {
		  if (enyo.$[inId]){
		  	console.warn(enyo.format("[enyo] Duplicated DOM id '%s'", inId), inControl, "will overwrite to", enyo.$[inId]);
		  }
		  enyo.$[inId] = inControl;
		},
