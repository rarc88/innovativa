...
static Logger log4j = Logger.getLogger(DocInvoiceData.class);
  ...
  public static DocInvoiceData[] select(ConnectionProvider connectionProvider, String invoice, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        '' AS FININVCOUNT, '' AS FINACCTCOUNT, '' AS FIN_PAYMENT_ID, '' AS C_COSTCENTER_ID, t.isCashVAT AS ISCASHVAT, '' AS PREPAYMENTAMT, '' AS ACCOUNT_ID" +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectDocInvoiceData.accountId = UtilSql.getValue(result, "account_id");
  ...
  public static DocInvoiceData[] selectBPartnerCustomerAcct(ConnectionProvider connectionProvider, String C_BPARTNER_ID, String C_ACCTSCHEMA_ID, String STATUS)    throws ServletException {
    return selectBPartnerCustomerAcct(connectionProvider, C_BPARTNER_ID, C_ACCTSCHEMA_ID, STATUS, 0, 0);
  }

  public static DocInvoiceData[] selectBPartnerCustomerAcct(ConnectionProvider connectionProvider, String C_BPARTNER_ID, String C_ACCTSCHEMA_ID, String STATUS, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT COALESCE((SELECT EM_SCNAC_NCPROVEEDOR " +
      "		  FROM C_BP_CUSTOMER_ACCT " +
      "          WHERE C_BPARTNER_ID = ?" +
      "          AND C_ACCTSCHEMA_ID = ?)," +
      "		  (COALESCE((SELECT C_RECEIVABLE_ACCT" +
      "          FROM C_BP_CUSTOMER_ACCT" +
      "          WHERE C_BPARTNER_ID = ?" +
      "          AND C_ACCTSCHEMA_ID = ?" +
      "          AND STATUS = ?)," +
