...
static Logger log4j = Logger.getLogger(DocInvoiceData.class);
  ...
  public String assetacct;
  public String istaxexpence;
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("ratetaxundeductable"))
      ...
      return iscashvat;
    else if (fieldName.equalsIgnoreCase("assetacct"))
      return assetacct;
    else if (fieldName.equalsIgnoreCase("istaxexpence"))
      return istaxexpence;
   else {
  ...
  public static DocInvoiceData[] select(ConnectionProvider connectionProvider, String invoice, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        '' AS FININVCOUNT, '' AS FINACCTCOUNT, '' AS FIN_PAYMENT_ID, '' AS C_COSTCENTER_ID, t.isCashVAT AS ISCASHVAT, '' AS ASSETACCT, '' AS ISTAXEXPENCE" +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectDocInvoiceData.assetacct = UtilSql.getValue(result, "assetacct");
        objectDocInvoiceData.istaxexpence = UtilSql.getValue(result, "istaxexpence");
  ...
  public static DocInvoiceData[] selectWithholdings(ConnectionProvider connectionProvider, String invoice)    throws ServletException {
    return selectWithholdings(connectionProvider, invoice, 0, 0);
  }

  public static DocInvoiceData[] selectWithholdings(ConnectionProvider connectionProvider, String invoice, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT t.IsTaxUndeductable as rateTaxUndeductable, (select IsTaxUndeductable from ad_orginfo o where o.ad_org_id=iti.ad_org_id) as orgTaxUndeductable, iti.C_Tax_ID, t.Name, t.Rate, t.IsTaxDeductable, SUM(iti.TaxBaseAmt) AS TaxBaseAmt, SUM(iti.TaxAmt) AS TaxAmt" +
      "        FROM C_INVOICELINE il LEFT JOIN C_INVOICETAX iti ON il.EM_SSWH_INVOICETAX_INCOME_ID = iti.C_INVOICETAX_ID," +
      "        C_TAX t" +
      "        WHERE iti.C_TAX_ID = t.C_TAX_ID AND il.C_INVOICE_ID = ?" +
      "        GROUP BY t.IsTaxUndeductable, iti.C_Tax_ID, t.Name, t.Rate, t.IsTaxDeductable, iti.ad_org_id" +
      "        UNION ALL" +
      "        SELECT t.IsTaxUndeductable as rateTaxUndeductable, (select IsTaxUndeductable from ad_orginfo o where o.ad_org_id=itv.ad_org_id) as orgTaxUndeductable, itv.C_Tax_ID, t.Name, t.Rate, t.IsTaxDeductable, SUM(itv.TaxBaseAmt) AS TaxBaseAmt, SUM(itv.TaxAmt) AS TaxAmt" +
      "        FROM C_INVOICELINE il LEFT JOIN C_INVOICETAX itv ON il.EM_SSWH_INVOICETAX_VAT_ID = itv.C_INVOICETAX_ID," +
      "        C_TAX t" +
      "        WHERE itv.C_TAX_ID = t.C_TAX_ID AND il.C_INVOICE_ID = ?" +
      "        GROUP BY t.IsTaxUndeductable, itv.C_Tax_ID, t.Name, t.Rate, t.IsTaxDeductable, itv.ad_org_id";
  ...
  public static DocInvoiceData[] selectLine(ConnectionProvider connectionProvider, String acctSchemaId, String invoice)    throws ServletException {
    return selectLine(connectionProvider, acctSchemaId, invoice, 0, 0);
  }

  public static DocInvoiceData[] selectLine(ConnectionProvider connectionProvider, String acctSchemaId, String invoice, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT il.em_sipc_is_tax_expence AS IstaxExpence, pa.P_Asset_Acct AS assetAcct, t.IsTaxUndeductable AS rateTaxUndeductable, (select IsTaxUndeductable from ad_orginfo o where o.ad_org_id=it.ad_org_id) AS orgTaxUndeductable, it.C_Tax_ID, t.Name, t.Rate, t.IsTaxDeductable, it.TaxBaseAmt AS TaxBaseAmt, it.TaxAmt AS TaxAmt" +
      "        FROM C_Tax t, C_InvoiceLineTax it" +
      "        LEFT JOIN C_invoiceLine il ON it.C_InvoiceLine_ID = il.C_InvoiceLine_ID" +
      "        LEFT JOIN M_Product_Acct pa ON il.M_Product_ID = pa.M_Product_ID AND pa.C_AcctSchema_ID = ?" +
      "        WHERE t.C_Tax_ID=it.C_Tax_ID AND it.C_Invoice_ID = ?";
  ...
  public static DocInvoiceData[] selectPayments(ConnectionProvider connectionProvider, String invoice, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT SUM(CASE WHEN ISPREPAYMENT = 'Y' THEN 0 ELSE FIN_PAYMENT_SCHEDULEDETAIL.AMOUNT END) AS AMOUNT,  CASE WHEN (FIN_PAYMENT_SCHEDULEDETAIL.FIN_PAYMENT_SCHEDULE_ORDER IS NOT NULL " +
      ...
      "        FIN_PAYMENT_SCHEDULE.C_CURRENCY_ID, SUM(CASE WHEN ISPREPAYMENT = 'Y' THEN FIN_PAYMENT_SCHEDULEDETAIL.AMOUNT ELSE 0 END) AS PREPAIDAMT" +
