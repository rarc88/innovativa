...
static Logger log4j = Logger.getLogger(DocLineGLJournalData.class);
  ...
  public String ssacctJournallineId;
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("ssacct_journalline_id") || fieldName.equals("ssacctJournallineId"))
      return ssacctJournallineId;
  ...
  public static DocLineGLJournalData[] select(ConnectionProvider connectionProvider, String ssacct_Journal_ID)    throws ServletException {
    return select(connectionProvider, ssacct_Journal_ID, 0, 0);
  }

  public static DocLineGLJournalData[] select(ConnectionProvider connectionProvider, String ssacct_Journal_ID, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT " +
      "            JL.SSACCT_JOURNALLINE_ID," +
      "            JL.AD_CLIENT_ID," +
      "            JL.AD_ORG_ID," +
      "            JL.LINE," +
      "            JL.DESCRIPTION," +
      ...
      "            JL.Qty," +
      "            J.C_ACCTSCHEMA_ID," +
      "            VC.C_ElementValue_ID AS C_VALIDCOMBINATION_ID," +
      "            JL.ACCOUNT_ID  AS M_C_GLITEM_ID," +
      "            '' as AD_ORGTRX_ID," +
      "            '' as C_LOCFROM_ID," +
      "            '' as C_LOCTO_ID," +
      "            COALESCE(JL.M_PRODUCT_ID, '') AS M_PRODUCT_ID," +
      "            COALESCE(JL.C_BPARTNER_ID, '') AS C_BPARTNER_ID," +
      "            COALESCE(JL.C_SALESREGION_ID,'') AS C_SALESREGION_ID," +
      "            COALESCE(JL.C_PROJECT_ID,'') AS C_PROJECT_ID," +
      "            COALESCE(JL.C_CAMPAIGN_ID,'') AS C_CAMPAIGN_ID," +
      "            COALESCE(JL.C_ACTIVITY_ID,'') AS C_ACTIVITY_ID," +
      "            COALESCE(JL.USER1_ID,'') AS USER1_ID," +
      "            COALESCE(JL.USER2_ID,'') AS USER2_ID," +
      ...
      "        FROM  " +
      "            SSACCT_JOURNALLINE JL INNER JOIN SSACCT_JOURNAL J ON J.SSACCT_JOURNAL_ID = JL.SSACCT_JOURNAL_ID " +
      "            LEFT JOIN C_ElementValue VC ON VC.C_ElementValue_ID=JL.C_VALIDCOMBINATION_ID" +
      "        WHERE  JL.SSACCT_JOURNAL_ID = ? AND JL.ISACTIVE='Y' AND (( " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, ssacct_Journal_ID);
      ...
      while(continueResult && result.next()) {
        ...
        objectDocLineGLJournalData.ssacctJournallineId = UtilSql.getValue(result, "ssacct_journalline_id");
