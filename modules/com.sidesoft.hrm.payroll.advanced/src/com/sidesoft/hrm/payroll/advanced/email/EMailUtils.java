 ...
 ******************************************************************************
 */
package com.sidesoft.hrm.payroll.advanced.email;
...
public class EMailUtils {
  ...
  public EMailUtils(VariablesSecureApp vars, String smtpHost, String from, String to) {
    g_from = from;
    addTo(to);
    g_smtpHost = smtpHost;
    g_vars = vars;
  ...
  public EMailUtils(VariablesSecureApp vars, String smtpHost, String from, String to, String subject,
      String message) {
    g_from = from;
    addTo(to);
    g_subject = subject;
  ...
  public EMailUtils(VariablesSecureApp vars, String smtpHost, String from, String to, String subject, String message, String port, boolean tls, boolean ssl) {
	    g_from=from;
	    addTo(to);
	    g_subject=subject;
	    g_message=message;
  ...
  public String send2(String user, String pass) {
	    StringBuffer sb = new StringBuffer();
	    Properties props = new Properties();
	    props.put("mail.debug", "true");
	    props.put("mail.store.protocol", "smtp");
	    ...
	    try {
	      ...
	      InternetAddress[] address = getTos();
	      if (address.length == 1) g_smtpMsg.setRecipient(Message.RecipientType.TO, address[0]);
	      else g_smtpMsg.setRecipients(Message.RecipientType.TO, address);
	      address = getCcs();
	      if (address != null && address.length > 0) g_smtpMsg.setRecipients (Message.RecipientType.CC, address);
	      address = getBccs();
	      if (address != null && address.length > 0) g_smtpMsg.setRecipients (Message.RecipientType.BCC, address);
	      if (g_replyTo != null) g_smtpMsg.setReplyTo(new Address[] {g_replyTo});
	      g_smtpMsg.setSentDate(new java.util.Date());
	      g_smtpMsg.setHeader("Comments", "CompiereMail");
	      g_smtpMsg.setAllow8bitMIME(true);
	      g_smtpMsg.setNotifyOptions(SMTPMessage.NOTIFY_FAILURE | SMTPMessage.NOTIFY_SUCCESS);
