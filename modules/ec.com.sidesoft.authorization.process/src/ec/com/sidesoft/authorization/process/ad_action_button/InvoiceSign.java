...
import ec.com.sidesoft.authorization.process.ad_action_button.InvoiceAuthorize;
import org.openbravo.model.common.invoice.Invoice;
import ec.com.sidesoft.authorization.process.AuthorizationInvoice;

public class InvoiceSign extends BaseActionHandler {

	private static final Logger log = Logger.getLogger(InvoiceSign.class);
	...
	protected JSONObject execute(Map<String, Object> parameters, String data) {
		...
		try {
			...
			final JSONArray invoiceIds = jsonData.getJSONArray("orders");
			final String action = jsonData.getString("action");
			tadFinal = jsonData.getString("tad");
			tableFinal = jsonData.getString("table");

			InvoiceAuthorize invoiceauthorize = new InvoiceAuthorize();

			for (int i = 0; i < invoiceIds.length(); i++) {
				// get the invoice
				final Invoice invoice = OBDal.getInstance().get(Invoice.class, invoiceIds.getString(i));
				getFileID(invoiceIds.getString(i));
				invoiceauthorize.createLineAuthorizationLevel(invoice,invoice.getEcsapAuthLevel(),reasonrejection);
				invoice.setEcsapSigned(true);
			}
