...
class BusinessPartnerMultipleData implements FieldProvider {
  ...
  public static BusinessPartnerMultipleData[] select(ConnectionProvider connectionProvider,
      String rownum, String adUserClient, String adUserOrg, String key, String name, String contact,
  ...
  public static BusinessPartnerMultipleData[] select(ConnectionProvider connectionProvider,
    ...
    strSql = strSql
        + ((codigoPostal == null || codigoPostal.equals("") || codigoPostal.equals("%")) ? ""
            : " AND UPPER(a.Postal) LIKE UPPER(?)  ");
    strSql = strSql + ((provincia == null || provincia.equals("") || provincia.equals("%")) ? ""
        : " AND UPPER(C_REGION.NAME) LIKE UPPER(?) ");
    ...
    try {
      ...
      while (continueResult && result.next()) {
        ...
        objectBusinessPartnerMultipleData.soCreditavailable = UtilSql.getValue(result,
            "so_creditavailable");
        objectBusinessPartnerMultipleData.soCreditused = UtilSql.getValue(result, "so_creditused");
        objectBusinessPartnerMultipleData.actuallifetimevalue = UtilSql.getValue(result,
            "actuallifetimevalue");
    ...
    BusinessPartnerMultipleData objectBusinessPartnerMultipleData[] = new BusinessPartnerMultipleData[vector
        .size()];
  ...
  public static String countRows(ConnectionProvider connectionProvider, String rownum,
    ...
    strSql = strSql + ((rownum == null || rownum.equals("")) ? "" : rownum);
    strSql = strSql + " AS RN1, B.* FROM ( select 1"
        + "            FROM C_BPARTNER_LOCATION l left join AD_USER c on l.C_BPartner_Location_ID = c.C_BPartner_Location_ID and c.isactive='Y'"
        + "                                       left join C_LOCATION a on l.C_Location_ID=a.C_Location_ID"
        + "                                       left join C_REGION on a.C_REGION_ID = C_REGION.C_REGION_ID,"
        + "                 C_BPARTNER bp" + "            WHERE bp.C_BPARTNER_ID = l.C_BPartner_ID"
        + "            AND bp.IsSummary='N'" + "            AND bp.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient == null || adUserClient.equals("")) ? "" : adUserClient);
    strSql = strSql + ")" + "            AND bp.AD_Org_ID IN (";
    strSql = strSql + ((adUserOrg == null || adUserOrg.equals("")) ? "" : adUserOrg);
