...
public class ProcessCertificate extends DalBaseProcess {
  OBError message;
  ...
  private void processRequest(ProcessBundle bundle) {
    ...
    } else if (certificate.getProcess().equals("N")
        && (!certificate.getCertificateStatus().equals("RE") && !certificate.getCertificateStatus().equals("RW"))) {
      message.setTitle(Utility.messageBD(conn, "SFB_CertificateWarning", language));
      message.setType("Warning");
      message.setMessage(Utility.messageBD(conn, "SFB_NoRequestedProcessStatus", language));
    } else if (certificate.getProcess().equals("Y")
    ...
    //Comprobar preferencia de revisión
      String strReviewCertificates="N";
      try{
    	  strReviewCertificates = Preferences.getPreferenceValue("Sfb_ReviewCertificates", true, new Client() , new Organization(), new User(), new Role(), new Window());
      }catch(Exception e){
    	  e.printStackTrace();
      }
      strReviewCertificates = strReviewCertificates == null ? "N" : strReviewCertificates;
      if(!strReviewCertificates.equals("N") && !strReviewCertificates.equals("Y")) {
        throw new OBException(Utility.messageBD(conn, "Sfb_PreferenceError", language));
      } 
    	
      if (certificate.getProcess().equals("N")) {
        ...
        if (strReviewCertificates.equals("Y")) {
	        certificate.setProcess("N");
