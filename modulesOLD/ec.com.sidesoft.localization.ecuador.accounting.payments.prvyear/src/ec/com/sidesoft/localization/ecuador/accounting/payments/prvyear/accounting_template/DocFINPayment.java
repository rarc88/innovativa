...
import ec.com.sidesoft.localization.ecuador.accounting.payments.prvyear.accounting_template.PaymentPreviousYearData;
...
public class DocFINPayment extends AcctServer {
    ...
    try {
      ...
      for (int i = 0; p_lines != null && i < p_lines.length; i++) {
        ...
        if ("".equals(line.getC_GLItem_ID())) {
          ...
          if(invoice != null){
                 ...
                 String strDateTimeFormat = (String) properties.get(KernelConstants.DATE_FORMAT_PROPERTY);
                 try {
      ...
      //   }else{
       // 	 previousyear = true;
        // }
            ...
            if (!isPrepayment) {
              if (line.getDoubtFulDebtAmount().signum() != 0) {
                BigDecimal doubtFulDebtAmount = convertAmount(line.getDoubtFulDebtAmount(),
                    ...
                    false);
                fact.createLine(line, getAccountBPartnerPayment(bpartnerId, as, true, false, true, conn,previousyear),
          ...
          fact.createLine(line, getAccountBPartnerPayment(bpartnerId, as, isReceipt, isPrepayment, conn, previousyear),
          ...
          if (line.isPrepaymentAgainstInvoice()) {
            ...
            if (!getAccountBPartnerPayment(bpartnerId, as, isReceipt, true, conn,previousyear).Account_ID
                .equals(getAccountBPartnerPayment(bpartnerId, as, isReceipt, false, conn, previousyear).Account_ID)) {
              fact.createLine(line2, getAccountBPartnerPayment(bpartnerId, as, isReceipt, false, conn, previousyear),
                  strcCurrencyId, (isReceipt ? "" : bpAmountConverted),
                  (isReceipt ? bpAmountConverted : ""), Fact_Acct_Group_ID3, nextSeqNo(SeqNo),
                  DocumentType, conn);	
              fact.createLine(line2, getAccountBPartnerPayment(bpartnerId, as, isReceipt, true, conn,previousyear),
                  strcCurrencyId, (!isReceipt ? "" : bpAmountConverted),
                  (!isReceipt ? bpAmountConverted : ""), Fact_Acct_Group_ID3, nextSeqNo(SeqNo),
                  DocumentType, conn);
            }
      ...
      if (new BigDecimal(usedAmount).compareTo(ZERO) != 0
        ...
        for (FIN_Payment_Credit creditPayment : creditPayments) {
           ...
           PaymentPreviousYear = PaymentPreviousYearData.selectPreviousYear(connectionProvider, creditPayment.getId());
          ...
          fact.createLine(
              null,
              getAccountBPartnerPayment(C_BPartner_ID, as, creditPayment.getCreditPaymentUsed()
                  .isReceipt(), true, conn, previousyear), creditPayment.getCreditPaymentUsed().getCurrency()
                  .getId(),
        ...
        if (creditPayments.isEmpty()) {
          fact.createLine(null,
              getAccountBPartnerPayment(C_BPartner_ID, as, payment.isReceipt(), true, conn, previousyear),
  ...
  public final Account getAccountBPartnerPayment(String cBPartnerId, AcctSchema as, boolean isReceipt,
	      boolean isPrepayment, ConnectionProvider conn, boolean previousyear) throws ServletException {
	    return getAccountBPartnerPayment(cBPartnerId, as, isReceipt, isPrepayment, false, conn, previousyear);
	  }
	   ...
	   */
	  public final Account getAccountBPartnerPayment(String cBPartnerId, AcctSchema as, boolean isReceipt,
	      boolean isPrepayment, boolean isDoubtfuldebt, ConnectionProvider conn, boolean previousyear)
	      throws ServletException {
