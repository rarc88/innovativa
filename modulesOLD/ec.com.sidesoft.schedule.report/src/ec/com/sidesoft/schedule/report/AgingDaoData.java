...
package ec.com.sidesoft.schedule.report;
...
class AgingDaoData implements FieldProvider, ScrollableFieldProvider {
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("bpid"))
      ...
      return cName;
    else if (fieldName.equalsIgnoreCase("usName"))
      return usName;
    else {
  ...
  public static AgingDaoData select(ConnectionProvider connectionProvider, String showDoubtfulDebt,
      String asOfDate, String currencyId, String firstRangeBucket, String secondRangeBucket,
      String thirdRangeBucket, String fourthRangeBucket, String org, String cbPartnerId,
      String recOrPay, String excludeVoids, String pgLimit, String oraLimit1)
    ...
    strSql = strSql
        + "         SELECT B.* FROM ("
        + "         SELECT A.bpId, bp.name as bpName, SUM(A.amount) as amount, A.scope, 0 as credit, '' as invoiceId, '' as docNo, '' as dateInvoiced, SUM(doubtfuldebtamt) as doubtfuldebt, A.cName, A.usName "
        + "         FROM ( "
        ...
        + "				         ELSE 5 "
        + "				        END                                                        AS scope, "
        + "			          cc.name as cName, us.name as usName "
        ...
        + "				       INNER JOIN C_Costcenter            cc  ON i.c_costcenter_id = cc.c_costcenter_id "
        + "				       INNER JOIN User1            us  ON us.User1_ID = i.User1_ID "
        + "				 WHERE psd.isactive = 'Y' " + "				   AND psd.iscanceled = 'N' "
        + "				   AND (psd.AD_Org_ID in (";
    strSql = strSql + ((org == null || org.equals("")) ? "" : org);
    strSql = strSql + ")) " + "				   AND (i.AD_Org_ID in (";
    strSql = strSql + ((org == null || org.equals("")) ? "" : org);
    strSql = strSql + ")) " + "				   AND 2=2";
    strSql = strSql
        + ((cbPartnerId == null || cbPartnerId.equals("")) ? "" : "  AND i.C_BPARTNER_ID IN"
            + cbPartnerId);
    strSql = strSql + "				   AND i.issotrx = ? " + "				   AND 1=1";
    strSql = strSql
        + ((excludeVoids.equals("excludeVoids")) ? " AND i.DOCSTATUS NOT IN ('VO') " : "");
    strSql = strSql
        ...
        + "			  ) A " + "			   INNER JOIN c_bpartner bp on (A.bpId = bp.c_bpartner_id)"
        + "		GROUP BY A.bpId, bp.name, A.scope, A.cName, A.usName  " + "        ) B";
    ...
    try {
      ...
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, showDoubtfulDebt);
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, asOfDate);
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, currencyId);
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, currencyId);
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, asOfDate);
      ...
      UtilSql.setValue(st, iParameter, 12, null, asOfDate);
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, firstRangeBucket);
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, secondRangeBucket);
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, thirdRangeBucket);
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, fourthRangeBucket);
      ...
      iParameter++;
      UtilSql.setValue(st, iParameter, 12, null, recOrPay);
  ...
  private AgingDaoData getselect() throws SQLException {
    ...
    objectAgingDaoData.cName = UtilSql.getValue(result, "cName");
    objectAgingDaoData.usName = UtilSql.getValue(result, "usName");
    objectAgingDaoData.rownum = Long.toString(countRecord);
    return objectAgingDaoData;
  }

  public static AgingDaoData selectCredit(ConnectionProvider connectionProvider, String currencyId,
      String org, String paidStatus, String asOfDate, String recOrPay, String cbPartnerId,
      String pgLimit, String oraLimit1) throws ServletException {
    AgingDaoData instance = new AgingDaoData();
    ...
    strSql = strSql
        ...
        + "                        ELSE fp.documentNo"
        + "                        END                             AS docNo"
        + "                FROM   fin_payment fp" + "                WHERE  (fp.AD_Org_ID in (";
    strSql = strSql + ((org == null || org.equals("")) ? "" : org);
    strSql = strSql + ")) " + "                       AND (fp.status IN (";
    strSql = strSql + ((paidStatus == null || paidStatus.equals("")) ? "" : paidStatus);
    strSql = strSql + "))" + "                       AND Trunc(fp.paymentdate) <= TO_DATE(?)"
        + "                       AND fp.isreceipt = ? " + "                       AND 1=1";
    strSql = strSql
        + ((cbPartnerId == null || cbPartnerId.equals("")) ? "" : "  AND fp.C_BPARTNER_ID IN"
            + cbPartnerId);
    strSql = strSql
    ...
    strSql = strSql + ((org == null || org.equals("")) ? "" : org);
    strSql = strSql + "))" + "                       AND (fp2.status IN (";
    strSql = strSql + ((paidStatus == null || paidStatus.equals("")) ? "" : paidStatus);
    strSql = strSql + "))" + "                       AND Trunc(fp2.paymentdate) <= TO_DATE(?)"
        + "                       AND fp2.isreceipt = ?" + "                       AND 2=2";
    strSql = strSql
        + ((cbPartnerId == null || cbPartnerId.equals("")) ? "" : "  AND fp2.C_BPARTNER_ID IN"
            + cbPartnerId);
    strSql = strSql
        + "                       AND fpsd.isactive = 'Y'"
        + "                       AND fpsd.iscanceled = 'N'"
    ...
    strSql = strSql
        ...
        + "       ) A" + "      INNER JOIN c_bpartner bp ON A.c_bpartner_id = bp.c_bpartner_id"
        + "   WHERE A.credit <> 0" + "        ) B";
    strSql = strSql + ((pgLimit == null || pgLimit.equals("")) ? "" : " LIMIT " + pgLimit);
    strSql = strSql
        + ((oraLimit1 == null || oraLimit1.equals("")) ? "" : " WHERE ROWNUM <= " + oraLimit1);
  ...
  public static AgingDaoData selectDetail(ConnectionProvider connectionProvider, String currencyId,
      String showDoubtfulDebt, String asOfDate, String firstRangeBucket, String secondRangeBucket,
      String thirdRangeBucket, String fourthRangeBucket, String org, String cbPartnerId,
      String recOrPay, String excludeVoids, String pgLimit, String oraLimit1)
      throws ServletException {
    AgingDaoData instance = new AgingDaoData();
    ...
    strSql = strSql
        ...
        + "           B.dateacct as dateInvoiced, FIN_AGING_INVOICECURRENCY_RATE(B.c_invoice_id, ?, B.c_currency_id) * (B.doubtfuldebtamt) as doubtfuldebt, B.cName,  B.usName "
        + "           FROM (  "
        + "                 SELECT A.c_invoice_id, A.c_currency_id, A.bpId, A.documentNo, A.dateacct, SUM(A.amount) as amount, SUM(A.doubtfuldebtamt) as doubtfuldebtAmt, A.scope, A.cName, A.usName "
        + "                 FROM ( "
    ...
    strSql = strSql
        ...
        + "                                                AND pd.fin_payment_detail_id = psd.fin_payment_detail_id) ) ) "
        + "                      ) A       "
        + "                GROUP BY A.c_invoice_id, A.c_currency_id, A.bpId, A.documentNo, A.dateacct, A.scope, A.cName, A.usName "
        + "               ) B"
