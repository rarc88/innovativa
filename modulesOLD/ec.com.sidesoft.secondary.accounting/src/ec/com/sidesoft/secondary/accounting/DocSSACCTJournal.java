...
public class DocSSACCTJournal extends AcctServer {
  private static final long serialVersionUID = 1L;
  static Logger log4jDocGLJournal = Logger.getLogger(DocSSACCTJournal.class);

  private String SeqNo = "0";
  private String C_AcctSchema_ID;

  public DocSSACCTJournal() {
  }
   ...
   *          client
   */
  public DocSSACCTJournal(String AD_Client_ID, String AD_Org_ID,
  ...
  public boolean loadDocumentDetails(FieldProvider[] data, ConnectionProvider conn) {
    ...
    try {
      ...
      log4jDocGLJournal.error("Error retrieving the accounting schema (general ledger)", e);
    }
    // Contained Objects
  ...
  private DocLine[] loadLines(ConnectionProvider conn) {
    ...
    try {
      ...
      for (int i = 0; i < data.length; i++) {
        String Line_ID = data[i].ssacctJournallineId;
        DocLine_SSACCTJournal docLine = new DocLine_SSACCTJournal(DocumentType, Record_ID, Line_ID);
        ...
        try {
          ...
          acct.Account_ID = C_ValidCombination_ID;
        } catch (ServletException e) {
  ...
  public Fact createFact(AcctSchema as, ConnectionProvider conn, Connection con,
    ...
    FactSecondary fact = new FactSecondary(this, as, m_PostingType);
      ...
      try {
        ...
        for (int i = 0; i < p_lines.length; i++) {
          if (Float.parseFloat(p_lines[i].getAmtSourceDr()) > Float.parseFloat(p_lines[i]
              .getAmtSourceCr())) {
            account = ((DocLine_SSACCTJournal) p_lines[i]).getAccount("1", as, conn);
          } else {
            account = ((DocLine_SSACCTJournal) p_lines[i]).getAccount("2", as, conn);
          }
  ...
  public static void setLog4jDocGLJournal(Logger log4jDocGLJournal) {
    DocSSACCTJournal.log4jDocGLJournal = log4jDocGLJournal;
  }
  ...
  public boolean getDocumentConfirmation(ConnectionProvider conn, String strRecordId) {
    ...
    if (STATUS_VOIDED.equals(OBDal.getInstance().get(SSACCTJOURNAL.class, strRecordId)
        .getDocumentStatus())) {
