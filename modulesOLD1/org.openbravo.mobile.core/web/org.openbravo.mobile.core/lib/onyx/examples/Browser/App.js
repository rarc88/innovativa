...
enyo.kind({
  ...
  components: [{
    ...
    components: [{
      ...
      components: [{
        ...
        kind: "onyx.IconButton",
        src: "images/menu-icon-back.png",
        ontap: "back",
        style: "float: left;"
      }, {
        kind: "onyx.IconButton",
        src: "images/menu-icon-forward.png",
        ontap: "forward",
      ...
      components: [{
        name: "bookmarks",
        kind: "FittableRows",
        style: "width: 300px; border-right: 1px solid silver;",
        components: [{
          name: "titleDecorator",
          kind: "onyx.InputDecorator",
          style: "display: block; margin: 4px; padding: 8px;",
          components: [{
            name: "titleInput",
            kind: "onyx.Input",
            placeholder: "Enter a title",
            style: "width: 210px; margin-right: 4px;"
          }, {
            kind: "onyx.IconButton",
            src: "images/menu-icon-add.png",
            ontap: "addBookmark",
            style: "float: right;"
          }
  ...
  rowItem: {
    kind: "Control",
    style: "border-bottom: 1px solid silver; padding: 10px;",
    ontap: "rowTap"
  },
  create: function () {
  ...
  renderBookmarks: function () {
    ...
    this.bookmarks = enyo.json.parse(OB.UTIL.localStorage.getItem(s));
    if (!this.bookmarks) {
      this.saveBookmarks(bookmarks);
      this.bookmarks = bookmarks;
    }
    for (var i = 0, c; c = this.bookmarks[i]; i++) {
      this.createComponent(this.rowItem, {
        container: this.$.list,
        content: c.title,
        url: c.url,
        index: i
  ...
  saveBookmarks: function (inBookmarks) {
    OB.UTIL.localStorage.setItem(this.storageItemName, enyo.json.stringify(inBookmarks));
  },
